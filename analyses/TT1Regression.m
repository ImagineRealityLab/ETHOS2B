...
img_vals = zscore(img_vals);
perc_vals = zscore(perc_vals);
...
% --------------------------
labels_train = meta.reproduction(img_idx);
meg_train    = all_trials_meg_3d(:,:,img_idx);
...
labels_test = meta.reproduction(perc_idx);
meg_test    = all_trials_meg_3d(:,:,perc_idx);
...
chance_level = 0;
yline(chance_level, 'k--', 'LineWidth', 1.5);

% Time zero line
xline(0, 'k--', 'LineWidth', 1.5);

title(sprintf('Ridge regression - Vividness <--> Visibility, N = %d', nValidSubj))
