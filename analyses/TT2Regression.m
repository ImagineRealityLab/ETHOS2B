...
% --------------------------
labels_train = meta.reproduction(img_yesTrials);
...
labels_test = [ones(length(best_RJ_present),1); zeros(length(best_RJ_absent), 1)];
...
performance = zeros(size(X_test,3),1);
...
for iRun = min(meta.run):max(meta.run)
    ...
    [perf,result] = mv_regress(cfg, X_train_sub, labels_train_sub, X_test_sub, labels_test_sub);
    
    time_axis = meg_all.time{1};
    performance = performance + perf;
end

perf = performance ./ nRun;
diag_perf = diag(perf);
...
xlabel('Time (s)');
ylabel('R-squared');
title(sprintf('Ridge regression - Vividness <--> RJ, N = %d', 5));
