...
perc_CR  = meta.condition == 0 & meta.presence == 0 & meta.detection == 0;
perc_FA  = meta.condition == 0 & meta.presence == 0 & meta.detection == 1;

perc_miss = meta.condition == 0 & meta.presence == 1 & meta.detection == 0;
perc_hit = meta.condition == 0 & meta.presence == 1 & meta.detection == 1;
...
if sum(classA) <2|| sum(classB) <2 || sum(classC) <2|| sum(classD) <2
...
% Smooth the data
X_test = movmean(X_test, 13, 3); % 20 ms temporal smoothing
...
legend('No imagery CR','No Imagery FA')
xlabel('Time (s)')
ylabel('Mean amplitude')
title(sprintf('Occipital ERF - CR/FA - Subject %d', iSubj))
...
legend('No imagery Miss','No imagery Hit')
xlabel('Time (s)')
ylabel('Mean amplitude')
title(sprintf('Occipital ERF - Miss/Hits - Subject %d', iSubj))
xline(-1.85, '--', 'First beep', 'LineWidth', 3, 'HandleVisibility','off')
xline(-0.95, '--', 'Second beep', 'LineWidth', 3, 'HandleVisibility','off')
xline(0, '--', 'Last beep', 'LineWidth', 3, 'HandleVisibility','off')
xlim([-2.4, 0.7])
...
% Mean line
plot(x, movmean(mean_diag, 2), 'Color', [0 0.447 0.741], 'LineWidth', 2);
...
xlabel('Time (s)');
ylabel('AUC');
title(sprintf('Diagonal decoding - No imagery CR/FA <--> No imagery Hit/Miss, N = %d', nValidSubj));
xline(-1.85, '--', 'First beep', 'LineWidth', 3, 'HandleVisibility','off')
xline(-0.95, '--', 'Second beep', 'LineWidth', 3, 'HandleVisibility','off')
xline(0, '--', 'Last beep', 'LineWidth', 3, 'HandleVisibility','off')
xlim([-2.4, 0.7])
