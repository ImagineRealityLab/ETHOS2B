...
classA = meta.orientation ==1 & meta.condition == 1 & meta.reproduction >= img_q75 | meta.orientation ==1 & meta.condition == 0 & meta.detection == 1 & meta.reproduction >= perc_q75;
classB = meta.orientation ==1 & meta.condition == 1 & meta.reproduction <= img_q25 | meta.orientation ==1 & meta.condition == 0 & meta.detection == 1 & meta.reproduction <= perc_q25;

classC = meta.orientation ==2 & meta.condition == 1 & meta.reproduction >= img_q75 | meta.orientation ==2 & meta.condition == 0 & meta.detection == 1 & meta.reproduction >= perc_q75;
classD = meta.orientation ==2 & meta.condition == 1 & meta.reproduction <= img_q25 | meta.orientation ==2 & meta.condition == 0 & meta.detection == 1 & meta.reproduction <= perc_q25;
...
% trainRunNo = meta.run(idx);
% testRunNo = meta.run([best_RJ_present;best_RJ_absent]);

% nRun = max(meta.run) - min(meta.run);
...
% Shaded error
plot_with_shade(x, movmean(mean_diag, 100), sd_diag, [0 0.447 0.741]);
...
xlabel('Time (s)');
ylabel('AUC');
title(sprintf('Diagonal decoding -Viv/Vis Grating A <--> Viv/Vis Grating B, N = %d', nValidSubj));
...
xlabel('Train Time (s)');
ylabel('Test Time (s)');
title(sprintf('TGM - Viv/Vis Grating A <--> Viv/Vis Grating B, N = %d', nValidSubj))
colormap(bluewhitered)
clim([0,1])   % symmetric around chance
colorbar_handle = colorbar;
label= ylabel(colorbar_handle, 'AUC', 'FontSize', 16, 'Rotation', 270);
