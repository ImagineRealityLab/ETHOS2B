...
HV_img_B  = meta.orientation==1 & meta.condition == 1 & meta.reproduction >= img_q75;
LV_img_B  = meta.orientation==1 & meta.condition == 1 & meta.reproduction <= img_q25;

HV_perc_B = meta.orientation==1 & meta.condition == 0 & meta.reproduction >= perc_q75;
LV_perc_B = meta.orientation==1 & meta.condition == 0 & meta.reproduction <= perc_q25;

classA = HV_img_B | HV_perc_B;
classB = LV_img_B | LV_perc_B;

HV_img_B  = meta.orientation==2 & meta.condition == 1 & meta.reproduction >= img_q75;
LV_img_B  = meta.orientation==2 & meta.condition == 1 & meta.reproduction <= img_q25;

HV_perc_B = meta.orientation==2 & meta.condition == 0 & meta.reproduction >= perc_q75;
LV_perc_B = meta.orientation==2 & meta.condition == 0 & meta.reproduction <= perc_q25;

classC = HV_img_B | HV_perc_B;
classD = LV_img_B | LV_perc_B;
...
xlabel('Train Time (s)');
ylabel('Test Time (s)');
title('Temporal Generalization - Viv/Vis Grating A -> Grating B');
...
legend('High-viv Grating A','Low viv Grating A')
xlabel('Time (s)')
...
xlim([-2.4, 0.7])
ylim([-2.500000000000000e-13, +2.500000000000000e-13])
...
legend('High-viv Grating B','Low viv Grating B')
xlabel('Time (s)')
ylabel('Mean amplitude')
