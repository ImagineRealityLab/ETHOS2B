...
function [V,acc] = StaircaseMEG(subID,orientation,gratingName,V,config, deviceToUse, volume)
...
gaborPatch = make_stimulus(orientation,1, 1); % full visibility
...
text = ['We will now do a calibration on the visibility of the gratings in noise. \n ',...
    'Your task is simply to indicate whether the grating below is presented (Yes [LR] or No [LI]) \n ',...
...
text = sprintf('You will detect Grating %s in this calibration. \n\n [Press any key to start]', gratingName);
...
%             schedule(onset_frame:offset_frame) = max(vis_scale(round(linspace(1,visibility,nSteps))));
   ...
   target = {};
    ...
    if stairCount==nEvalTrials
        ...
        fprintf('Accuracy for this stair: %.3f\n',acc(stairStep));
