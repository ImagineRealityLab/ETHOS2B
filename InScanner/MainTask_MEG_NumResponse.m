...
fprintf('Measured IFI: %.6f s | Refresh rate: %.2f Hz\n', ifi, hz);
...
text = ['Please let the experimenter know if you may have any questions at this point. This phase consists of 16 blocks \n'...,
    ...
    'We will start the experiment in a second!'];
...
for iBlock = startBlock:nBlocks
   ...
   % instruction screen with gratings
    ...
    for iTrial = TrialLoopStart:trialsPerBlock
            ...
            itiDur = ITIs(total_trial_counter);
            
            % Show ITI screen immediately
            Screen('DrawLines', w, [0 0 -10 10; -10 10 0 0], ...
                4, [255,255,255], [rect(3)/2, rect(4)/2], 1);
            vbl = Screen('Flip', w);
            
            % Build textures while ITI screen stays visible
            target = cell(1, nSteps);
            schedule_no = trials(iBlock,iTrial,2);
            
            if trials(iBlock,iTrial,1) == 1
            ...
            for i_frame = 1:nSteps
                ...
                tmp = Screen('MakeTexture', w, ...
                    make_stimulus_differentNoises( ...
                        orientations(blocks(iBlock,1)), ...
                        schedule(i_frame), ...
                        schedules{schedule_no}{1, i_frame}));
            ...
            % Wait only for the remaining ITI time
            elapsed = GetSecs - vbl;
            remainingITI = itiDur - elapsed;
            if remainingITI > 0
                WaitSecs(remainingITI);
            end
        ...
        for i_frame = 1:nSteps
           ...
           elseif i_frame == onset_frame
            ...
            vbl = Screen('Flip', w, vbl + (nStepSize - 0.5) * ifi);
