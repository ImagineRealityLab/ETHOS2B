...
if p > 0.5
    image_region = mask(:, :, 1) > 0;

    % Start with gray background everywhere
    noisePattern(:) = 255/2;

    % Inside image region:
    % - some pixels are image
    % - others are noise
    noisePattern(image_region & take_image_value) = image(image_region & take_image_value);
    noisePattern(image_region & ~take_image_value) = noisePattern(image_region & ~take_image_value);
end
